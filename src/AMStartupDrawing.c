...
#define NUM_EXEC_ARGS 10  	// number of arguments needed for CHILD_EXEC
...
int main(int argc, char *argv[]) {
    ...
    //socket to link to child process to do actual drawing
    GtkWidget *gtkSocket = gtk_socket_new();
    gtk_container_add(GTK_CONTAINER(window), gtkSocket);
    ...
    gtk_window_set_default_size(GTK_WINDOW(window), ntohl(serverMessage.init_ok.MazeWidth)*CELL_SIZE, ntohl(serverMessage.init_ok.MazeHeight)*CELL_SIZE);
    gtk_widget_show_all(window);
    //get socket ID to pass to drawing avatar
    GdkNativeWindow socketID = gtk_socket_get_id(GTK_SOCKET(gtkSocket));
    long unsigned int sktID = socketID;
    //start main processs
    gtk_main();
...
//	char *socketId = calloc(11,sizeof(char));
